...
add_dat_4area <- function(
   ...
   fleet_key <- data.frame(fleet = seq_len(nrow(dat_template$fleetinfo)),
                           fleetname = dat_template$fleetinfo$fleetname)
   tmp_catch <- tidyr::pivot_longer(dat_4A_4$catch, cols = seq_len(dat_4A_4$Nfleet),
                                    names_to = "fleetname")
   tmp_catch <- merge(x = tmp_catch, y = fleet_key)
   tmp_catch <- tmp_catch[, c("year", "seas", "fleet", "value")]
   colnames(tmp_catch)[4] <- "catch"
   ...
   dat_out$catch <- tmp_catch
   # CPUE
   tmp_CPUE <- type.convert(dat_4A_4$CPUE[, 1:5], as.is = TRUE)
   tmp_CPUE <- tmp_CPUE %>%
      rename(obs = cpu) %>%
      rename(catch_se = cv) %>%
      select(year, seas, index, obs, catch_se)
   dat_out$CPUE <- tmp_CPUE
   # length comps
   dat_out$lencomp <- dat_4A_4$lencomp
   # tag data
   dat_out$tag_recaps <- dat_4A_4$tag_recaps
   dat_out$tag_releases <- dat_4A_4$tag_releases
   dat_out$N_tag_groups <- dat_4A_4$N_tag_groups
   dat_out$N_recap_events <- nrow(dat_4A_4$tag_recaps)
   dat_out
}
